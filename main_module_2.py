...
def main() -> _text:
    ...
    optimizer: _optimizor =  torch.optim.SGD
    criterion: _criterion = nn.CrossEntropyLoss()
    ...
    class ImageClassificationBase(nn.Module):
        def training_step(self, batch: int) -> float:
            images, labels = batch 
            out: Any = self(images)                  
            loss: float = F.cross_entropy(out, labels) 
            return loss
        ...
        def validation_epoch_end(self, outputs: Any) -> dict[str, int]:
            batch_losses: list = [x['val_loss'] for x in outputs]
            epoch_loss: Container = torch.stack(batch_losses).mean()   # Combine losses
            batch_accs: list = [x['val_acc'] for x in outputs]
            epoch_acc: float = torch.stack(batch_accs).mean()      # Combine accuracies
    ...
    @torch.no_grad()
    def evaluate(model: _model, val_loader: _loader) -> dict[str, float]:
        model.eval()
        outputs: list[float] = [model.validation_step(batch) for batch in val_loader]
        return model.validation_epoch_end(outputs)



    def get_lr(optimizer: _optimizer) -> Any:
        for param_group in optimizer.param_groups:
            return param_group['lr']



    def fit_one_cycle(epochs: int, max_lr: float, 
                                model: _model, 
                                train_loader: _loader, 
                                val_loader: _loader, 
                                weight_decay: Optional[float|int] = 0, 
                                grad_clip: Optional[float] = None, 
                                opt_func: Optional[_optimizer] = torch.optim.SGD) -> _text:
        torch.cuda.empty_cache()
        history: list = []
        
        optimizer: _optimizer = opt_func(model.parameters(), max_lr, weight_decay=weight_decay)
        ...
        for epoch in range(epochs): 
            ...
            for batch in train_loader:
                ...
                #@: appending amd updating learning rate
